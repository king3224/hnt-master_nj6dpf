...
import com.shtoone.njshtw.adapter.MaterialStatisticFragmentRecyclerViewAdapter;
import com.shtoone.njshtw.adapter.WaterStablityMaterialStatisticFragmentRecyclerViewAdapter;
 ...
 */
public class WaterStablityMaterialStatisticFragment extends BaseLazyFragment {
    private static final String TAG = WaterStablityMaterialStatisticFragment.class.getSimpleName();
    private Toolbar                                                   mToolbar;
    private NestedScrollView                                          mNestedScrollView;
    private PageStateLayout                                           mPageStateLayout;
    private PtrFrameLayout                                            mPtrFrameLayout;
    private Typeface                                                  mTf;
    private WaterStablityMaterialStatisticFragmentRecyclerViewAdapter mAdapter;
    private FloatingActionButton                                      fab;
    private RecyclerView                                              mRecyclerView;
    private boolean isRegistered = false;
    private BarChart mBarChart0, mBarChart1;
    private WaterStablityMaterialStatisticFragmentData data;
    private ParametersData                             mParametersData;
    private Gson                                       mGson;
    private LinearLayoutManager                        mLinearLayoutManager;

    public static WaterStablityMaterialStatisticFragment newInstance() {
        return new WaterStablityMaterialStatisticFragment();
    }
...
//        initToolbarMenu(mToolbar);
    ...
    public String createRefreshULR() {
        ...
        return URL.getCailiaoyongliang(userGroupID, startDateTime, endDateTime);
    }
    ...
    public void onRefreshSuccess(String response) {
        if (!TextUtils.isEmpty(response)) {
            ...
            if (jsonObject.optBoolean("success")) {
                data = mGson.fromJson(response, WaterStablityMaterialStatisticFragmentData.class);
                if (null != data) {
    ...
    private void setViewData() {
        ...
        List<WaterStablityMaterialStatisticFragmentData.DataEntity> mList = data.getData();
        for (int i = 0; i < mList.size(); i++) {
            x.add(mList.get(i).getName());
            if (!TextUtils.isEmpty(mList.get(i).getYongliang())&&!TextUtils.isEmpty(mList.get(i).getMbpeibi())){
        ...
        //设置动画与适配器
        SlideInLeftAnimationAdapter mSlideInLeftAnimationAdapter = new SlideInLeftAnimationAdapter(mAdapter = new WaterStablityMaterialStatisticFragmentRecyclerViewAdapter(_mActivity, data.getData()));
