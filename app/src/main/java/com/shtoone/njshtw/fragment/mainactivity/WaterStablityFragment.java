package com.shtoone.njshtw.fragment.mainactivity;
...
import com.shtoone.njshtw.activity.DialogActivity;
import com.shtoone.njshtw.activity.MainActivity;
import com.shtoone.njshtw.activity.OrganizationActivity;
import com.shtoone.njshtw.activity.PitchActivity;
import com.shtoone.njshtw.activity.WaterStablityActivity;
import com.shtoone.njshtw.adapter.OnItemClickListener;
import com.shtoone.njshtw.adapter.PitchFragmentRecyclerViewAdapter;
import com.shtoone.njshtw.adapter.WaterStablityFragmentRecyclerViewAdapter;
import com.shtoone.njshtw.bean.DepartmentData;
import com.shtoone.njshtw.bean.ParametersData;
import com.shtoone.njshtw.bean.PitchFragmentRecyclerViewItemData;
import com.shtoone.njshtw.bean.WaterStablityFragmentRecyclerViewItemData;
import com.shtoone.njshtw.fragment.base.BaseLazyFragment;
import com.shtoone.njshtw.ui.PageStateLayout;
import com.shtoone.njshtw.utils.AnimationUtils;
import com.shtoone.njshtw.utils.ConstantsUtils;
import com.shtoone.njshtw.utils.DensityUtils;
 ...
 */
public class WaterStablityFragment extends BaseLazyFragment {

    private static final String TAG = WaterStablityFragment.class.getSimpleName();
    private Toolbar                                   mToolbar;
    private PtrFrameLayout                            mPtrFrameLayout;
    private RecyclerView                              mRecyclerView;
    private WaterStablityFragmentRecyclerViewAdapter  mAdapter;
    private WaterStablityFragmentRecyclerViewItemData itemsData;
    private FloatingActionButton                      fab;
    private PageStateLayout                           mPageStateLayout;
    private ParametersData                            mParametersData;
    private Gson                                      mGson;
    private DepartmentData                            mDepartmentData;
    public static WaterStablityFragment newInstance() {
        return new WaterStablityFragment();
    }
    ...
    private void initData() {
        if (null != BaseApplication.parametersData) {
            ...
            mParametersData.fromTo = ConstantsUtils.WATERSTABLITYFRAGMENT;
        }
        if (null != BaseApplication.mDepartmentData && !TextUtils.isEmpty(BaseApplication.mDepartmentData.departmentName)) {
            mDepartmentData = new DepartmentData(BaseApplication.mUserInfoData.getDepartId(), BaseApplication.mUserInfoData.getDepartName(), ConstantsUtils.WATERSTABLITYFRAGMENT);
        }
    ...
    public String createRefreshULR() {
        ...
        return URL.getSWLingdaoData(userGroupID, startDateTime, endDateTime);
    }

    @Override
    public void onRefreshSuccess(String response) {
        KLog.e(TAG,response);
        Log.e("waterstablity",response);
        if (!TextUtils.isEmpty(response)) {
            ...
            if (jsonObject.optBoolean("success")) {
                itemsData = mGson.fromJson(response, WaterStablityFragmentRecyclerViewItemData.class);
                if (null != itemsData) {
    ...
    public void updateSearch(ParametersData mParametersData) {
        ...
        if (mParametersData != null) {
            if (mParametersData.fromTo == ConstantsUtils.WATERSTABLITYFRAGMENT) {
                fab.show();
    ...
    public void updateDepartment(DepartmentData mDepartmentData) {
        if (null != mDepartmentData && null != mParametersData && null != this.mDepartmentData) {
            if (mDepartmentData.fromto == ConstantsUtils.WATERSTABLITYFRAGMENT) {
    ...
    private void setToolbarTitle() {
        if (null != mToolbar && null != mDepartmentData && !TextUtils.isEmpty(mDepartmentData.departmentName)) {
            ...
            sb.append(getString(R.string.waterstablity)).trimToSize();
    ...
    private void setAdapter() {
        ...
        //设置动画
        SlideInLeftAnimationAdapter mSlideInLeftAnimationAdapter = new SlideInLeftAnimationAdapter(mAdapter = new WaterStablityFragmentRecyclerViewAdapter(_mActivity, itemsData));
    ...
    private void jumpToPitchActivity(int position) {
        Intent intent = new Intent(_mActivity, WaterStablityActivity.class);
        //这里不需要做健壮性判断，因为能走到这一步，itemsData必然有数据
        BaseApplication.mDepartmentData.departmentID = itemsData.getData().get(position).get(0).getDeptId();
        BaseApplication.mDepartmentData.departmentName = itemsData.getData().get(position).get(0).getDeptName();
        startActivity(intent);
    }
}
