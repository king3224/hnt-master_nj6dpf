...
//    public static final String BHZ_CHAOBIAO_SP = BaseURL + "app.do?AppHntChaobiaoShenpi&jieguobianhao=%1&jianliresult=%2&jianlishenpi=%3&confirmdate=%4&shenpiren=%5&shenpidate=%6";
    ...
    */
    ...
    public static String getTanpuSudu(String userGroupID, String startTime, String endTime, String shebeibiaohao,String pageNo,String maxPageItems){
        ...
        //如果开始时间大于结束时间，返回null
        if (Integer.valueOf(startTime) <= Integer.valueOf(endTime)) {
            String url = TANPU_SUDU_URL.replace("%1", userGroupID).replace("%2", startTime).replace("%3", endTime).replace("%4", shebeibiaohao).replace("%5", pageNo).replace("%6", maxPageItems);
            KLog.e(TAG, "摊铺速度查询URL :" + url);
            if (TextUtils.isEmpty(url)) {
                return null;
            }
            return url;
    ...
    public static String getOutletTemp(String userGroupID, String startTime, String endTime, String shebeibiaohao,String pageNo,String maxPageItems){
        ...
        //如果开始时间大于结束时间，返回null
        if (Integer.valueOf(startTime) <= Integer.valueOf(endTime)) {
            String url = OUTLET_TEMP_URL.replace("%1", userGroupID).replace("%2", startTime).replace("%3", endTime).replace("%4", shebeibiaohao).replace("%5", pageNo).replace("%6", maxPageItems);
            KLog.e(TAG, "出料口温度查询URL :" + url);
            if (TextUtils.isEmpty(url)) {
                return null;
            }
            return url;
     ...
     * 水稳主页url
     */
    public static final String waterstablity_Lingdao_Tongji = BaseURL + "swController.do?swChaoBiaoCount&userGroupId=%1&startTime=%2&endTime=%3";
     ...
     * @return 返回拼凑后的url
     */
    public static String getSWLingdaoData(String userGroupID, String startTime, String endTime) {
        startTime = DateUtils.ChangeTimeToLong(startTime);
        endTime = DateUtils.ChangeTimeToLong(endTime);
        //如果开始时间大于结束时间，返回null
        if (Integer.valueOf(startTime) <= Integer.valueOf(endTime)) {
            String url = waterstablity_Lingdao_Tongji.replace("%1", userGroupID).replace("%2", startTime).replace("%3", endTime);
            KLog.e(TAG, "水稳主界面URL :" + url);
            if (TextUtils.isEmpty(url)) {
                return null;
            }
            return url;
     ...
     * 请求水稳生产数据查询列表数据
     */
    public static final String SW_Produce_Query = BaseURL + "swScsjcxController.do?getScsjcxList&userGroupId=%1&shebeibianhao=%2&startTime=%3&endTime=%4&pageNo=%5";
     ...
     * @return url
     */
    public static String getSWProduceData(String userGroupID, String deviceNo, String startTime, String endTime, String current_PageNo) {
        startTime = DateUtils.ChangeTimeToLong(startTime);
        endTime = DateUtils.ChangeTimeToLong(endTime);
        deviceNo = DateUtils.ChangeTimeToLong(deviceNo);
        //如果开始时间大于结束时间，返回null
        if (Integer.valueOf(startTime) <= Integer.valueOf(endTime)) {
            String url = SW_Produce_Query.replace("%1", userGroupID).replace("%2", deviceNo).replace("%3", startTime).replace("%4", endTime).replace("%5", current_PageNo);
            KLog.e(TAG, "水稳生产数据查询URL :" + url);
            if (TextUtils.isEmpty(url)) {
                return null;
            }
            return url;
     ...
     * 请求沥青生产数据查询数据详情页
     */
    public static final String SW_Produce_Query_Detail = BaseURL + "swScsjcxController.do?getScsjcxDetail&shuiwenid=%1&shebeibianhao=%2";
     ...
     * @return 返回拼凑后的url
     */
    public static String getSWProduceDetailData(String bianhao,String shebeibianhao) {
        String url = SW_Produce_Query_Detail.replace("%1", bianhao).replace("%2", shebeibianhao);
        KLog.e(TAG, "水稳生产数据详情查询 :" + url);
        if (TextUtils.isEmpty(url)) {
            return null;
        }
        return url;
     ...
     * 混凝土材料用量
     */
    public static final String SW_CAILIAO_URL = BaseURL + "swScsjcxController.do?swmaterial&deptId=%1&startTime=%2&endTime=%3";
     ...
     * @return 返回拼凑后的url
     */
    public static String getCailiaoyongliang(String userGroupID, String startTime, String endTime) {
        startTime = DateUtils.ChangeTimeToLong(startTime);
        endTime = DateUtils.ChangeTimeToLong(endTime);
        //如果开始时间大于结束时间，返回null
        if (Integer.valueOf(startTime) <= Integer.valueOf(endTime)) {
            String url = SW_CAILIAO_URL.replace("%1", userGroupID).replace("%2", startTime).replace("%3", endTime);
            KLog.e(TAG, "水稳材料用量URL :" + url);
            if (TextUtils.isEmpty(url)) {
                return null;
            }
            return url;
     ...
     * 水稳待处置超标列表查询
     */
    public static final String SW_CHAOBIAO_LIST_URL = BaseURL + "swScsjcxController.do?chaoBiaoList&startTime=%1&endTime=%2&shebeibianhao=%3&chaobiaolx=%4&cllx=%5&deptId=%6&pageNo=%7&maxPageItems=10";
     ...
     * @return 返回拼凑后的url
     */
    public static String getSWChaobiaoListData(String startTime, String endTime, String shebeibianhao, String chaobiaolx, String cllx, String deptId, String pageNo) {
        startTime = DateUtils.ChangeTimeToLong(startTime);
        endTime = DateUtils.ChangeTimeToLong(endTime);
        String url = SW_CHAOBIAO_LIST_URL.replace("%1", startTime).replace("%2", endTime).replace("%3", shebeibianhao).replace("%4", chaobiaolx).replace("%5", cllx).replace("%6", deptId).replace("%7", pageNo);
        KLog.e(TAG, "沥青超标等级查询 :" + url);
        Log.e("水稳超标等级查询",url);
        if (TextUtils.isEmpty(url)) {
            return null;
     ...
     * 水稳超标处置详情页uil
     */
    public static final String SW_CHAOBIAOCHUZHI_DETAIL_URL = BaseURL + "swScsjcxController.do?chaoBiaoXQ&shuiwenid=%1&shebeibianhao=%2";
     ...
     * @return 返回拼凑后的url
     */
    public static String getSWOverProofDetailData(String shuiwenid, String shebeibianhao) {
        String url = SW_CHAOBIAOCHUZHI_DETAIL_URL.replace("%1", String.valueOf(shuiwenid)).replace("%2", shebeibianhao);
        Log.e(TAG, "沥青超标详情查询 :" + url);
        if (TextUtils.isEmpty(url)) {
            return null;
        }
        return url;
