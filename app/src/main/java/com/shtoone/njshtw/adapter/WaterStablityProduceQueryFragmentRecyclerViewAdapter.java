...
public class WaterStablityProduceQueryFragmentRecyclerViewAdapter extends RecyclerView.Adapter<ViewHolder> {
    private static final String TAG = WaterStablityProduceQueryFragmentRecyclerViewAdapter.class.getSimpleName();
    private Context                                      context;
    private OnItemClickListener                          mOnItemClickListener;
    private List<WaterStablityProduceQueryData.DataEntity> itemsData;
    ...
    public WaterStablityProduceQueryFragmentRecyclerViewAdapter(Context context, List<WaterStablityProduceQueryData.DataEntity> itemsData) {
        super();
    ...
    public void onBindViewHolder(final ViewHolder holder, int position) {
        if (holder instanceof ItemViewHolder) {
            ...
            WaterStablityProduceQueryData.DataEntity item = itemsData.get(position);
            mItemViewHolder.tv_datetime.setText(item.getShijians());
            mItemViewHolder.tv_site.setText(item.getBanhezhanminchen());
            mItemViewHolder.tv_project_name.setText(item.getShebeibianhao());
            mItemViewHolder.tv_position.setText(item.getSjshui());
            mItemViewHolder.tv_location.setText(item.getGlchangliang());
    ...
    public ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        if (viewType == ITEM_TYPE.TYPE_ITEM.ordinal()) {
            return new ItemViewHolder(LayoutInflater.from(context).inflate(R.layout.item_recyclerview_waterstablityproduce_query_fragment, parent, false));
