 ...
 */
public class SWOverproofDetailActivityRecyclerViewAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    private static final String TAG = SWOverproofDetailActivityRecyclerViewAdapter.class.getSimpleName();
    private Context                                context;
    private PitchProduceQueryDetailActivityData    itemsData;
    private Resources                              mResources;
    private List<WaterStablityOverProofDetailData.SwDataEntity> lists;

    public SWOverproofDetailActivityRecyclerViewAdapter(Context context, List<WaterStablityOverProofDetailData.SwDataEntity> lists) {
        super();
...
//          mViewGroup.setBackgroundColor(position % 2 == 0 ? mResources.getColor(R.color.material_teal_50) : mResources.getColor(R.color.material_blue_50));
        WaterStablityOverProofDetailData.SwDataEntity sc_chaxunItem_xq_data = lists.get(position);
        ...
        mItemViewHolder.tv_matching.setText(lists.get(position).getScpeibi());
        mItemViewHolder.tv_deviation.setText(lists.get(position).getSgpeibi());
